...
'''
Generating datasets of track
sources:
github.com/juangallostra/procedural-tracks/blob/master/main.py
en.wikipedia.org/wiki/Cenrtipetal_Catmull-Rom_spline
gamasutra.com/blogs/GustavoMaciel/20131229/207833/Generating_Procedural_Racetracks.php
stackoverflow.com/questions/30844482/what-is-most-efficient-way-to-find-the-intersection-of-a-line-and-a-circle-in-py
'''
...
# genereting inner part of the track
...
# finding intersection of circle and line
def intersection(center, radius, pt1, pt2, full_line=False, tangent_tol=1e-4):
    ...
    if discriminant < 0:
        ...
        intersections = [(cx + (big_d * dy + sign * (-1 if dy < 0 else 1) * dx * discriminant ** .5) / dr ** 2,
                          cy + (-big_d * dx + sign * abs(dy) * discriminant ** .5) / dr ** 2) for sign in
                         ((1, -1) if dy < 0 else (-1, 1))]
        if not full_line:
...
# genereting the outer wall of the track
...
#generating spline from the points
...
# function for pushing points apart so there aren't any that are too close to each other
...
# making sure the angles in the track are okey and the car will be able to race in the generated track
