...
#tohle se může odstranit - dělá to jen to, že v simulaci se dají nastavit parametry přes posuvný panel
...
camInfo = p.getDebugVisualizerCamera() #přijde mi zajímavé, že je to před tím while - nedává to trochu smysl
#pokud to budeme chtít používat, bude se to muset updatovat
lastTime = time.time()
...
while (True):
	...
	#tahle část vyresluje ty snimky z kamery
	nowTime = time.time()
	...
	#tahle část je mega důležitá - zajištujě řízení auta	
	#control at 100Hz
	if (nowControlTime-lastControlTime>.01):
		...
		yaw = camInfo[8]
		pitch = camInfo[9]
		distance = camInfo[10] #vzdálenost objektu v kameře a kamery
		targetPos = camInfo[11] #souřadnice objektu v kameře
		camFwd = camInfo[5] #osa směřující dopředu
		carYaw = getCarYaw(car)

        #zajištění správného otáčení kol - fyzikálně správné otáčení
        ...
        #tady by to mělo přijímat info z učení - tyhle 3 řádky se jinak můžou odstranit
		maxForce = p.readUserDebugParameter(maxForceSlider)
		targetVelocity = p.readUserDebugParameter(targetVelocitySlider)
		steeringAngle = p.readUserDebugParameter(steeringSlider)
		#print(targetVelocity)
		#dává ryhlost oboum kolům
		for wheel in wheels:
		...
		steering    #pořád netuším na co tady tohle je
		if (useRealTimeSim==0):
			frame+=1
			p.stepSimulation() #důležité - dělá krok v simulaci
